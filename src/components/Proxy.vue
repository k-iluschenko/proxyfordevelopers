<template>
  <section class="container">
    <div class="row">
      <div class="sidebar  col-md-3 col-sm-4 form">
        ...
        <div class="form__alive">
          <div>
            <input v-model="checkedAlive" class="form-check-input" type="checkbox" value="" id="aliveCheck">
        ...
        <table class="table table-striped table-bordered">
          <thead class="thead-dark">
            <tr>
              ...
              <th class="disable_mobile_screen" scope="col">Country</th>
              <th class="disable_table_screen" scope="col">Proxy type</th>
              <th class="disable_table_screen" scope="col">Last check time</th>
          ...
          <tbody>
            <tr v-for="(proxy, index) in filterProxies" :key='proxy.id'>
              ...
              <td class="disable_mobile_screen">{{proxy.country}}</td>
              <td class="disable_table_screen">{{proxyType(proxy.proxy_type)}}</td>
              <td class="disable_table_screen">{{new Date(proxy.last_check_time).toLocaleString()}}</td>
              <td>{{proxy.alive}}</td>
            </tr>
          </tbody>
        </table>
...
export default {
  ...
  methods: {
    ...
    clearFilter(){
     ...
     proxyType(typeId){
      ...
      let typeString ="";
      switch (typeId) {
        case 0 : typeString = 'Transparent';
        break;
         case 1 : typeString = 'Anonymous';
        break;
         case 2 : typeString = 'Elite';
        break;
      }
      return typeString
    }
  ...
  computed:{
    ...
    filterProxies(){
      ...
      let filterCountry=[], filterAlive=[], filterType=[];
      ...
      // фильтр по стране
      filterCountry = filterType.length && country !== "" ? filterType.filter(item => item.country === this.selectedCountry) : filterType      
      filterAlive = filterCountry.filter(item => item.alive === this.checkedAlive) 
      return filterAlive
    },
  }
